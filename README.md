...
result.snore_count        // 60 秒窗口内正例累计，可以用这个指标来demo，有鼾声都启动什么，不建议产品使用这个指标
result.iferr_count        // 60 秒窗口内干扰扣分累计
result.output_p           // 过滤后的鼾声指数（人讲话，放音乐，会扣分），即snore_count-iferr_count，建议采用这个来卡鼾声干预，基于这个大小来调整灵敏度
result.dreame_snore_index // 纯推理的鼾声次数
